...
//        System.out.println(toStringLot(root));
    ...
    //被删除节点和父节点都是黑色节点,即删除黑色节点后，达不到条件5，所以需要旋转或者着色变换
    private static void reBalance(Node node) {
        Node parent = node.getParent();
        //剩下的节点
        boolean leftPosition = node.getLeft() == null ? false : true;
        Node son = leftPosition ? parent.getRight() : parent.getLeft();
        Node sonLeft = son.getLeft();
        Node sonRight = son.getRight();
        if (son.isBlack()) {
            if (sonLeft != null || sonRight != null) {
                if (sonLeft != null && sonRight != null) {
                    if (leftPosition) {
                        sonLeft.setBlack(true);
                        rRotate(node);
                    } else {
                        sonRight.setBlack(true);
                        lRotate(node);
                    }
            ...
            if (leftPosition) {
                rRotate(parent);
                if (sonRight.getLeft() != null) {
                    sonRight.setBlack(false);
                    sonRight.getLeft().setBlack(true);
                    rRotate(parent);
                } else if (sonRight.getRight() != null) {
                    lRotate(sonRight);
                    rRotate(node);
                }
            } else {
                lRotate(node);
                if (sonLeft.getRight() != null) {
                    sonLeft.setBlack(false);
                    sonLeft.getRight().setBlack(true);
                    lRotate(node);
                } else if (sonLeft.getLeft() != null) {
                    rRotate(sonLeft);
                    lRotate(node);
                }
            }
