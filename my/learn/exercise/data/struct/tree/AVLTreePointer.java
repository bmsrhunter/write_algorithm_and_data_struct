...
public class AVLTreePointer {
    ...
    public AVLTreePointer(int[] array) {
        init(array);
    }
    ...
    private Node rRotate(Node node) {
        ...
        Node grandLeft = leftSon.getLeft();
        Node rightSon = node.getRight();
        int nv = node.getValue();
        int lv = leftSon.getValue();
        node.setValue(lv);
        Node newRight = new Node(nv);
        if (grandRight != null) {
            grandRight.setParent(newRight);
        }
        newRight.setLeft(grandRight);
        newRight.setRight(rightSon);
        newRight.setParent(node);
        if (rightSon != null) {
            rightSon.setParent(newRight);
        }
        node.setRight(newRight);
        grandLeft.setParent(node);
        node.setLeft(grandLeft);
        if (grandLeft == null) {
            node.setDiff(-1);
        } else {
            node.setDiff(0);
    ...
    private Node lRotate(Node node) {
        ...
        Node grandRight = rightSon.getRight();
        Node leftSon = node.getLeft();
        int nv = node.getValue();
        int rv = rightSon.getValue();
        node.setValue(rv);
        Node newLeft = new Node(nv);
        if (grandLeft != null) {
            grandLeft.setParent(newLeft);
        }
        newLeft.setRight(grandLeft);
        newLeft.setLeft(leftSon);
        newLeft.setParent(node);
        if (leftSon != null) {
            leftSon.setParent(newLeft);
        }
        node.setLeft(newLeft);
        grandRight.setParent(node);
        node.setRight(grandRight);
        if (grandRight == null) {
            node.setDiff(1);
        } else {
            node.setDiff(0);
        ...
        if (pnode != null) {
            ...
            if (leaf) {//删除叶子节点
                ...
                if (right == null) {
                    ...
                    PositionNode pn = findMaxNodeByLeft(node);
                    Node nd = pn.getNode();
                    int val = nd.getValue();
                    node.setValue(val);
    ...
    private PositionNode findMaxNodeByLeft(Node node) {
        if (node == null) {
            return null;
        }
        Node left = node.getLeft();
...
//        int[] numbers = {1, 3, 2};
        ...
        AVLTreePointer tree = new AVLTreePointer(numbers);
